...
export async function fetchSurveyLogsByAp(apId) {
  if (!isSupabaseConfigured) {
    await delay(150);
    return MOCK_SURVEY_LOG.filter((l) => l.ap_id === apId)
      .slice()
      .sort((a, b) => (b.survey_date || "").localeCompare(a.survey_date || "") || b.id - a.id);
  }
...
/**
 * 조사이력(survey_log)의 특정 회차를 실제로 수정합니다.
 * `submitSurvey`(현장조사 입력)와 달리 새 이력을 추가하지 않고, 지정한 회차(logId)를
 * 그대로 고칩니다(UPDATE). 단, 수정한 회차가 해당 AP의 "가장 최근 조사"라면
 * 목록/상세 화면에 보이는 ap_detail의 최신 상태도 함께 갱신해 일관성을 유지합니다.
 * 가장 최근 회차가 아니면 survey_log만 수정되고 ap_detail에는 영향이 없습니다.
 *
 * @param {Object} params
 ...
 * @param {File|null} [params.photoFile] - 사진을 교체할 경우에만 전달
...
export async function updateSurveyLog({ logId, apId, surveyDate, deviceStatus, networkStatus, remark, photoFile }) {
  if (!isSupabaseConfigured) {
    await delay(300);
    const target = MOCK_SURVEY_LOG.find((l) => l.id === logId);
    if (!target) throw new Error("조사이력을 찾을 수 없습니다.");

    target.survey_date = surveyDate;
    target.device_status = deviceStatus;
    target.network_status = networkStatus;
    target.remark = remark;
    if (photoFile) target.survey_photo_path = `ap${apId}_${surveyDate}(demo-edit)`;

    const logsForAp = MOCK_SURVEY_LOG.filter((l) => l.ap_id === apId);
    const latest = logsForAp
      .slice()
      .sort((a, b) => (b.survey_date || "").localeCompare(a.survey_date || "") || b.id - a.id)[0];
    const syncedLatest = latest?.id === logId;

    if (syncedLatest) {
      const apTarget = MOCK_AP_DETAIL.find((a) => a.id === apId);
      if (apTarget) {
        apTarget.device_status = deviceStatus;
        apTarget.network_status = networkStatus;
        apTarget.survey_date = surveyDate;
        apTarget.remark = remark;
        if (photoFile) apTarget.survey_photo_path = target.survey_photo_path;
      }
    }
    return { log: { ...target }, syncedLatest };
  }

  let surveyPhotoPath;
  if (photoFile) {
    const ext = photoFile.name?.split(".").pop() || "jpg";
    // ⚠️ Storage key에는 한글 등 비ASCII 문자를 쓸 수 없어 ASCII-safe한 apId/logId만 사용합니다.
    const path = `ap${apId}_log${logId}_${surveyDate}_${Date.now()}.${ext}`;
  ...
  // 이 AP의 가장 최근 조사 회차를 확인해, 지금 수정한 회차가 최신이면
  // ap_detail(목록/상세 화면에 보이는 최신 상태)도 함께 동기화합니다.
  ...
  const syncedLatest = latestLog?.id === logId;
  if (syncedLatest) {
  ...
  return { log: updatedLog, syncedLatest };
}
